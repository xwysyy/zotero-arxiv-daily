...
def rerank_paper(candidate:list[ArxivPaper],corpus:list[dict],model:str='avsolatorio/GIST-small-Embedding-v0') -> list[ArxivPaper]:
    ...
    collection_weight = np.array([0.3 if any(p.startswith('old/') for p in paper.get('paths', [])) else 1.0 for paper in corpus])
    weight = time_decay_weight * collection_weight
    weight = weight / weight.sum()
    corpus_feature = encoder.encode([paper['data']['title'] + '. ' + paper['data']['abstractNote'] for paper in corpus])
    candidate_feature = encoder.encode([paper.title + '. ' + paper.summary for paper in candidate])
